...
import AboutSecTwo from '../../section/activity/inner/AboutSecTwo';
import Filter from '../../section/activity/inner/Filter';
...
const ActivityDetails = () => {
  ...
  const getDestinations = async () => {
    setActivity('');
    const activityBySlug = await activityApi.activityBySlug(params.slug);

    if (activityBySlug.status === 200 && activityBySlug.data.status === 200 && activityBySlug.data.success === true) {
      await setActivity(activityBySlug.data.data);
    ...
    {loading ? '' : (
      <>
        <Box className='aboutBan relative'>
          <Parallax bgImage={`${process.env.REACT_APP_HOST_IMAGE}image/activities/banner/${activity.activitiesImage}`} bgImageAlt="" strength={100}>
            <Box sx={{
              flexGrow: 1,
              display: 'flex',
              alignItems: 'center',
            ...
            >
              ...
              >
                <Grid item xs={12} md={8}>
                  ...
                  >
                    {/* {parse(activity.description).props.children.slice(0, 150)} */}
        ...
        <AboutSecTwo activity={activity} />
        <Filter activityDetails={activity} destination={destinationList} difficulty={difficultyList} />
      </>
    )}
    </>
  )
